...
### 模型名称
`WiFiDensePoseModel` — 端到端 CSI→姿态模型，位于 `wifi-densepose-train/src/model.rs`
...
1. 子载波插值:  114 → 56 (线性插值, 统一输入尺寸)
                 对C5应该设为: 484 → 242 或保持原生242

2. 滑动窗口:    每100帧CSI → 1个训练样本
...
特征提取 (features.rs)                 ModalityTranslator
  │                                      │
  ├─ 呼吸率 (Fresnel + FFT)             ▼
  ├─ 心率 (BVP + 带通滤波)            ResNet18 Backbone
  ├─ 运动检测 (振幅方差)                 │
  ├─ 存在检测 (自适应阈值)              ▼
  └─ 人员定位 (三角测量)              DensePose输出
  │                                   (17关键点 + UV映射)
  ▼                                      │
START 分诊 (规则引擎)                     ▼
  │                                   3D骨架可视化
...
当前模型默认 56 子载波（适配 S3）。C5 的 20MHz 有 242 子载波：
...
# 2. 修改配置
# crates/wifi-densepose-train/src/config.rs
