...
- [WiFi DensePose 用户指南](#wifi-densepose-用户指南)
  ...
  - [数据源](#数据源)
    ...
    - [ESP32-C5/S3(完整CSI)](#esp32-c5s3完整csi)
...
| 选项 | 成本 | 能力 |
|--------|------|-------------|
| ESP32-C5/S3网格(3-6块板) | ~$54 | 完整CSI:姿态、呼吸、心跳、存在 |
...
ESP32节点通过UDP向端口5005流式传输二进制CSI帧。参见[硬件设置](#esp32-c5s3网格)了解刷写说明。

### ESP32多静态网格(高级)

为了更高的穿墙跟踪精度,以**多静态网格**配置部署3-6个ESP32-C5/S3节点。每个节点既充当发射器又充当接收器,通过环境创建多条感知路径。
...
| 手动加密回退 | 受限ESP32-C5/S3节点上的HMAC-SHA256信标认证 |
...
**要求：**
- 支持CSI的硬件（ESP32-C5/S3或研究用NIC）以获得准确读数
...
> [!TIP]
> **ESP32-C5 推荐** — C5 支持 WiFi 6 (802.11ax)，提供更高的 CSI 分辨率和更精准的感知能力。详见 [C5 固件](firmware/esp32-c5-csi-node/)。

3-6节点ESP32-C5/S3网格以20 Hz提供完整CSI。总成本：3节点设置约$54。

**所需物品：**
- 3-6个ESP32-C5/S3开发板（每个约$8）
- WiFi路由器（CSI源）
- 运行感知服务器的计算机（聚合器）
...
**问：支持哪些WiFi芯片组？**
答：
- **完整CSI**：ESP32-C5/S3（推荐）、Intel 5300、Atheros AR9580（仅Linux）
