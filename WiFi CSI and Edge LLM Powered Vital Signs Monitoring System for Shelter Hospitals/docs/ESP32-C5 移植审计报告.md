...
可进入编译验证阶段：在 IDF v5.5+ 环境 `idf.py set-target esp32c5 && idf.py build`。
...
| # | 文件 | 问题 | 修复 |
|---|------|------|------|
| 1 | `sdkconfig.defaults` | `CONFIG_ESP_WIFI_ENABLE_WIFI6=y` 不存在 | 已删除（WiFi 6 对 C5 target 隐式启用） |
...
| 6 | `Kconfig.projbuild` | 显示 GPIO(38/47/48) C5 不存在 | 已重配 C5 范围(0-21)+默认关闭显示 |
...
| 13 | `esp32_parser.rs` | `MAX_SUBCARRIERS=256` 会拒绝 C5 帧 | 已改 512 |
...
> **✅ 全部 5 个 P0/P1 问题已在审计后立即修复。** 以下保留原始分析供参考。
...
**问题：** C5 WiFi 6 20MHz 有 242 子载波，40MHz 有 484 子载波。当前值 128 只有 C5 实际的一半不到，会导致 edge_processing 只处理前 128 个子载波，丢失大量数据。
...
**问题：** C5 单天线 40MHz HE 的 IQ 数据可达 484 × 2 = 968 字节。如果 C5 的 CSI 回调返回多天线数据，或者包含 `first_word_invalid` 前缀字节，1024 字节可能不够。这会导致 `ring_push()` 截断数据。
...
### 🔴 问题 3：Rust Parser `MAX_SUBCARRIERS = 256` — 会拒绝 C5 帧

**文件：** `rust-port/wifi-densepose-rs/crates/wifi-densepose-hardware/src/esp32_parser.rs`
...
**文件：** `main/csi_collector.c` → `csi_serialize_frame()`
...
当前只有 `sdkconfig.qemu`（S3 用 Xtensa QEMU）。C5 需要 RISC-V QEMU 对应的配置。
...
| `install.sh` | S3 工具链/路径 | 需添加 C5 选项 |
...
| 🔴 P0 | #3 Rust MAX_SUBCARRIERS | 服务端拒绝 C5 帧 | 2 分钟 |
...
3. **编译验证** — 在 IDF v5.5+ 环境 `idf.py set-target esp32c5 && idf.py build`
4. **实机测试** — 2.4GHz 先测，再上 5GHz
5. **CI/文档** — 批量更新
...
| 18 | `sdkconfig.defaults.template` | ✅ 更新为 C5 target |
| 19 | `sdkconfig.defaults.4mb` | ✅ 更新为 C5 target + 4MB 配置 |
